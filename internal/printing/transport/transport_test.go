...
// The CONTRACT all four transports of §8.2 honour: the name they announce, the device key
// they really read, what they refuse at construction, what the probe reports, and what
// they do with a cancelled context or a close.
//
// The last two sections are what no double would prove: the production seams, exercised on
// a real file and a real socket, with no printer.
//
// The `file` transport, which is read back off the disk, has its own file.
