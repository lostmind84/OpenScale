...
// This file is the TYPED VOCABULARY of §8.3: one value type per SBPL field, each
// refusing at construction what the manual does not accept, plus the bounds and the
// operation names those refusals are spelled from. <A3> is the one field held apart, in
// offset.go, because its real bound is measured on the ink of the bitmap.
