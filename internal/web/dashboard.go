// This file holds THE ONE SCREEN A VOLUNTEER LOOKS AT when something is wrong: the
// hardware, the print routing, the roll, the disk, the catalogue in force and the
// findings it raised.
//
// It gathers from three places that can each be slow or absent -- the platform, the
// store, the hub -- and none of them is allowed to hold the page: every fill is
// bounded, and a source that does not answer leaves its block empty rather than
