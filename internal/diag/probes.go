...
func (m hostMachine) KioskTask(ctx context.Context) (ServiceState, error) {
	...
	case "windows":
		...
		return ServiceState{Name: windowsKioskTask, Known: err == nil, Determined: true,
			Detail: firstLine(out)}, nil
	case "linux":
...
func parseWindowsService(name, query, config string) ServiceState {
	...
	state.Detail = word
	if startType := tokenAfter(config, "START_TYPE"); startType != "" {
...
//
// `sc query` prints « STATE              : 4  RUNNING » and `sc qc` prints
// « START_TYPE         : 2   AUTO_START », so the token that carries the meaning is the
// last one on the line. The numeric code is deliberately ignored: the word is the same in
// every locale and the number is one lookup table away from being wrong.
