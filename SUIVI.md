...
| 7 | `internal/diag/doctor_service.go`, le remède du contrôle « service », et §11.1 la ligne « Journaux texte » | Renvoient vers `C:\ProgramData\OpenScale\data\logs\openscale.log`, qu'**aucun code n'écrit** : le journal technique vit en base (`technical_log`) et sort par le fichier de diagnostic (`technical.csv`) ou la page Journal ; sous Linux, `journalctl -u openscale`. Relevé le 04/09/2026 en cherchant où lire un journal sur un poste de production |
