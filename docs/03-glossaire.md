...
| `ERR-CFG-02` | `web.codeNoPassword` — le garde d'`authenticated` et l'ouverture de session | « Ce poste n'a pas encore de mot de passe. Saisissez le code de secours de la fiche d'installation pour en poser un. » **Ce code existe parce que le statut ne suffit pas** : 409 est aussi la réponse d'un compte à rebours déjà armé, d'une confirmation que personne n'attend et d'une mise à jour sur un poste occupé. L'écran ouvrait son panneau « code de secours + nouveau mot de passe » sur le statut nu, si bien qu'un double appui sur *Confirmer* réclamait la fiche d'installation à un exploitant authentifié depuis dix minutes. Le front lit désormais le code (`api.CODE_NO_PASSWORD`), et lui seul. |
